...
const page = () => {
  ...
  const handelLogIn = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (data.name === "") {
      setIsInput({ ...isInput, name: true });
    }
    if (data.username === "" && data.password === "" && data.name === "") {
      setIsInput({ username: true, password: true, name: true });
    }
    if (data.username !== "" && data.password !== "" && data.name !== "") {
        ...
        <form>
          <div className="flex justify-between items-center space-x-1.5 ">
            ...
            <input
              ...
              value={data.name}
              name="name"
              className={`border-2 font-mono ${isInput.name ? "border-red-500" : "border-black"}`}
              required
              onChange={(e) => {
                setData({ ...data, name: e.target.value });
                setIsInput({ ...isInput, name: false });
              }}
            />
          </div>
          <br />
